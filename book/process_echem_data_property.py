...
INPUT_FILE = "C_H_oxidation_ dataset_prime_data.xlsx"    # change if your Excel file has a different name
NOISE_STD_PKA = 0.25
NOISE_STD_TOX = 0.12
NOISE_STD_LOGS = 0.30
NOISE_STD_MP = 20.0          # melting point noise (°C)
...
def toy_logS_from_smiles(smiles: str,
    ...
    logS = 0.5 - 0.01 * (150.0 - 25.0) - logp  # assume MP=150 °C fallback
...
def toy_pKa_single(smiles: str,
    ...
    return round(float(np.clip(pick, -2.0, 15.0)), 2)
...
def toy_melting_point(smiles: str,
                      noise_std: float = NOISE_STD_MP,
                      deterministic: bool = DETERMINISTIC_NOISE) -> float:
    """
    Linear with MW: MP ≈ 0.5*MW + 20 °C + noise
    """
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return np.nan
    mw = Descriptors.MolWt(mol)
    mp = 0.5 * mw + 20.0
    mp += _seeded_gauss(smiles + "|mp", noise_std) if deterministic else random.gauss(0, noise_std)
    return round(float(mp), 1)
...
def process_file(input_folder: Path, input_filename: str):
    ...
    if "SMILES" not in df.columns:
        raise ValueError("Input must have a 'SMILES' column")
    props = df["SMILES"].astype(str).apply(
        lambda smi: pd.Series({
            ...
            "MeltingPoint_C": toy_melting_point(smi),
        })
    )
    smiles_idx = df.columns.get_loc("SMILES")
    for col in ["Solubility_logS_25C", "pKa_est", "Tox_Label", "MeltingPoint_C"]:
        df.insert(smiles_idx + 1, col, props[col])
        smiles_idx += 1
